...
inTrainingData <- caret::createDataPartition(y= Species, p=0.70, list = FALSE)
trainData <- iris[inTrainingData,]
testData <- iris[-inTrainingData,]

#Build the model on the training data
dtModel <- C50::C5.0(trainData[,-5], trainData$Species)
plot(dtModel)
...
# Ans: 88.88% cases were correctly classified
100* mean(testData$Species == predict(dtModel, testData))
